...
# 2. Load data
df = pd.read_csv("Location1.csv")   # change to whatever Location needed
...
df["lag1"] = df["Power"].shift(1)       # 1-hour lag
df["lag24"] = df["Power"].shift(24)     # 24-hour lag
df["target24"] = df["Power"].shift(-24) # next-day target
df = df.dropna()
...
print("Hour-ahead MAE:", mean_absolute_error(y_test, y_pred_hour))
print("Hour-ahead RMSE:", mean_squared_error(y_test, y_pred_hour, squared=False))
...
print("Day-ahead MAE:", mean_absolute_error(y_test, y_pred_day))
print("Day-ahead RMSE:", mean_squared_error(y_test, y_pred_day, squared=False))
...
# 7. Estimate lost MWh and financial cost
